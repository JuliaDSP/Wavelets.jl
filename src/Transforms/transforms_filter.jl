...
@propagate_inbounds function filtermainloop!(si, silen, b, val)
    for j = 2:silen
        si[j-1] = si[j] + b[j] * val
    end
    si[silen] = b[silen+1] * val
    return nothing
end
@propagate_inbounds function filtermainloopzero!(si, silen)
    for j = 2:silen
        si[j-1] = si[j]
    end
    si[silen] = 0.0
